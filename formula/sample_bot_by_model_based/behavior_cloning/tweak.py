...
def send_control(steering_angle_, throttle_, nsamples=0):
    print('angle:', steering_angle_, 'throttle:', throttle_, 'samples:', nsamples)
    sio.emit('steer', data={
...
def telemetry(sid, data):
    ...
    speed = float(data['speed'])/2
    
    prediction = model.predict([np.array([img]).astype('float'), np.array([speed]).astype('float')])
    ...
    if lr_val != 0 or ud_val != 0:
        ...
        speed_corr.append(speed)
        y_corr.append([angle_, throttle_])
    # sample old model
    ...
    # control car
    send_control(angle_, throttle_, len(X_corr))
...
def signal_handler(signal, frame):
    global X_corr, speed_corr, y_corr, screen
    print('Found', len(X_corr), 'to train with')

    if len(X_corr) > 0:
        X_corr = np.array(X_corr, dtype='float')
        speed_corr = np.array(speed_corr, dtype='float')
        y_corr = np.array(y_corr, dtype='float')
        print('Training with', len(X_corr), 'samples')
        ...
        model.fit([X_corr, speed_corr], y_corr,
                  batch_size=128,
                  nb_epoch=10,
                  verbose=1)
