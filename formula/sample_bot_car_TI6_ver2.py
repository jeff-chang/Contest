...
import logging
import tempfile
from sample_bot_by_track_detection.track_detection.twQTeamImageProcessor import twQTeamImageProcessor
...
class Log(object):
    def __init__(self,is_debug=True):
        ...
        hdlr = logging.FileHandler('./logs/formula_logs.log')
        ...
        if len(self.steering_variant) >= 5:
            ...
            if probability >= 0.2 or probability2>0.7:
                ...
                if self.current_increase != -1:
                    increase_value = math.fabs(self.current_increase - 0.03)
                    self.current_increase = -1
                #print "break"
...
class ImageProcessor(object):
    ...
    @staticmethod
    def get_non_line_count(sr, sg, sb):
        black_count = 0
        yellow_count = 0
        for i in range(len(sr) / 10):
            for j in range(len(sr[i]) / 10):
                inverse_i = len(sr)/6 + i
        ...
        return black_count, yellow_count
    ...
    def get_object_count(sr, sg, sb):
        ...
        for i in range(len(sr) / 10):
            for j in range(len(sr[i]) / 10):
                ...
                #inverse_i = i
                #inverse_j = j
                #print "r:{},g:{},b:{}".format(sr[inverse_i][inverse_j], sg[inverse_i][inverse_j], sb[inverse_i][inverse_j])
                if sr[inverse_i][inverse_j] == 0 and sg[inverse_i][inverse_j] == 0 and sb[inverse_i][inverse_j] == 0:
                    object_count += 1
        return object_count
    ...
    def find_steering_angle_by_color(img, last_steering_angle, debug = True):
        ...
        black_count,yellow_count=ImageProcessor.get_non_line_count(sr, sg, sb)
        #object_count=ImageProcessor.get_object_count(sr, sg, sb)
        ...
        if ImageProcessor.ALREADY_TRANSED==False:
            if black_count>=320:
                #print "Near the left wall:{}".format(black_count)
                if ImageProcessor.is_translating==False:
                    ...
                    ImageProcessor.track_index_current=0
                            #break
            if yellow_count >= 40:
                #print "Near the right wall:{}".format(yellow_count)
                if ImageProcessor.is_translating == False:
                    ImageProcessor.is_translating = True
                    #sorted(tracks)[1]
                    # target_value = sorted(tracks)[1]
                    ...
                    ImageProcessor.track_index_current = 0
                            #break
        #print "Right wall:{}, Left Wall:{}".format(black_count, yellow_count)
        ...
        if  ImageProcessor.is_translating and ImageProcessor.current_step<ImageProcessor.TRANSLATION_STEPS:
            ...
            #ImageProcessor.track_index_current=ImageProcessor.AUTO_DETECT
        ...
        #print "sorted(tracks):{}".format(sorted(tracks))
        #print "maximum_color_idx:{}".format(maximum_color_idx)
        if ImageProcessor.track_index_current!=-1:
            maximum_color_idx=ImageProcessor.track_index_current
...
class AutoDrive(object):
    ...
    def change_track(self, src_img, track_no):
        #TrackDetector
        ...
        if track_no == current_track_idx:
            ...
            if current_track_idx == -1:
                pass
            elif track_no < current_track_idx:
    ...
    def on_dashboard(self, src_img, last_steering_angle, speed, throttle, info):
        ...
        self.change_track(src_img, 4)
        print self.direction
        print self.direct_status
        if self.direction == 0:
            if self.frame_ignore:
                self._car.control(0.04 * self.direction, 0.005)
                self.frame_ignore = self.frame_ignore - 1
            else:
...
class Car(object):
    ...
    def tunner(self, steering_angle, throttle, switch):
        if not switch:
            return 1, 0
        if steering_angle > 35 or steering_angle < -35:
            angle_time = 1.4
            throttle_time = 0.008
        elif steering_angle > 30 or steering_angle < -30:
            angle_time = 1.3
            throttle_time = 0.018
        elif steering_angle > 20 or steering_angle < -20:
            angle_time = 1.2
            throttle_time = 0.028
        elif (steering_angle <= 5 and steering_angle >= -5): #or (steering_angle >= -20 and steering_angle <= -10):
           angle_time = 0.3
           throttle_time = 0.04
        else:
            angle_time = 0.55
            throttle_time = 0.03
        return angle_time, throttle_time
    ...
    def control(self, steering_angle, throttle):
        ...
        angle_time, throttle_time = self.tunner(steering_angle, throttle, False)
        self._control_function(steering_angle * angle_time, throttle + throttle_time)
