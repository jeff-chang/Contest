...
#
# AI Car Sample bot for Python 3 - TW Site Q Team
...
class ImageProcessor(object):
    ...
    def find_steering_angle_by_color(img, last_steering_angle, debug = True):
        ...
        track_list   = [sr, sg, sb]
        tracks       = np.asarray([len(track[track > 20]) for track in track_list])
        tracks_seen  = np.asarray(tracks[tracks > 50])
...
class AutoDrive(object):
    ...
    def on_dashboard(self, src_img, last_steering_angle, speed, throttle, info):
        ...
        if self.debug:
            ImageProcessor.show_image(np.asarray(src_img), "source")
            ImageProcessor.show_image(np.asarray(track_img), "track")
        ...
        if self._record_folder:
            ...
            ImageProcessor.save_image(self._record_folder, np.asarray(src_img)  , prefix = "cam", suffix = suffix)
            ImageProcessor.save_image(self._record_folder, np.asarray(track_img), prefix = "trk", suffix = suffix)
...
class Car(object):
    ...
    def on_dashboard(self, dashboard):
        ...
        img                 = cv2.imdecode(np.fromstring(base64.b64decode(dashboard["image"]), dtype=np.uint8), 1)
